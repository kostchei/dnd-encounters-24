...
const SOURCE_FILE = path.join(__dirname, '../data/Adventures/Waterdeep_Deungeon_of_The_Mad_Mage.md');
const BESTIARY_DIR = path.join(__dirname, '../5etools-v2.23.0/data/bestiary');
const OUTPUT_FILE = path.join(__dirname, '../data/mad_mage_encounters.json');
...
async function main() {
    ...
    for (const file of bestiaryFiles) {
        ...
        if (data.monster) {
            for (const m of data.monster) {
                // Key by name lower case
                bestiaryMap.set(m.name.toLowerCase(), m);

                // Also add logic to handle "Name (Variant)" keys if useful, but exact match first
            }
        }
    }
    ...
    for (const entry of parsedMonsters) {
        ...
        // Manual mapping
        const NAME_MAP = {
            "Giant Lizard (variant)": "Giant Lizard",
            "Troll (variant)": "Troll",
            "Drider (variant)": "Drider",
            "Big Xorn": "Xorn",
            "Armored Troglodyte": "Troglodyte", // Or "Troglodyte (Armored)" if valid, but fallback to Troglodyte
            "Giant Mimic": "Large Mimic",
            "Shadowy Duplicate": "Shadow", // Fallback if not found
            "Deformed Duergar": "Duergar"
        };

        if (NAME_MAP[name]) {
            bestiaryData = bestiaryMap.get(NAME_MAP[name].toLowerCase());
        }

        // Try stripping parenthesis
        if (!bestiaryData && name.includes('(')) {
            const stripped = name.replace(/\s*\(.*?\)\s*/g, '').trim();
            bestiaryData = bestiaryMap.get(stripped.toLowerCase());
        }
        ...
        if (bestiaryData) {
            ...
            outputList.push({
                ...
                Tags: [],
                Region: ["Dungeons"],
                Adventures: ["Waterdeep: Dungeon of the Mad Mage"]
            });
        } else {
            console.warn(`Missing stats for: ${name}`);
            missing.push(name);
            // Fallback object with tags
            outputList.push({
                Name: name,
                CR: entry.cr,
                Type: "Unknown",
    ...
    console.log(`Missing stats for ${missing.length} monsters.`);
