...
const SOURCE_FILE = path.join(__dirname, '../data/Adventures/TombOfAnnihilation.md');
const BESTIARY_DIR = path.join(__dirname, '../5etools-v2.23.0/data/bestiary');
const OUTPUT_FILE = path.join(__dirname, '../data/tomb_encounters.json');
...
async function main() {
    ...
    while (i < lines.length) {
        if (isCR(lines[i])) {
            ...
            // We only need name and CR to match with bestiary
            if (name) {
                parsedMonsters.push({
                    name: name,
                    cr: cr
    ...
    for (const entry of parsedMonsters) {
        ...
        // Manual mapping
        const NAME_MAP = {
            "Volothamp “Volo” Geddarm": "Volothamp \"Volo\" Geddarm", // Curly to straight quotes
            "Yuan-ti Malison": "Yuan-ti Malison (Type 1)", // Default to Type 1
            "Gray Slaad (control gem variant)": "Gray Slaad",
